...
from .adapt import fine_tune_vit, inject_adaptformer
...
BIRD_EVAL_CACHE_ROOT = Path("/vol/bitbucket/mm6322/selective_pruning_camera_ready/selective_pruning_mmd/exp21-ablation/cache/bird_eval")
...
def build_bird_eval_loaders(dataset, bird_labels, n_per_label, batch_size, transform=None):
    ...
    """
    global FORGET_SET_KEY, RETAIN_SET_KEY
    if not bird_labels:
    ...
    if not is_indexable:
        ...
        canon = ",".join(str(int(v)) for v in sorted(set(bird_labels)))
        label_encoding = hashlib.sha1(canon.encode()).hexdigest()[:10]
        head_split_name = f"birds_first_{n_per_label}_per_label_{label_encoding}"
        ...
        tail_split_name = f"birds_after_first_{n_per_label}_per_label_{label_encoding}"
        RETAIN_SET_KEY = tail_split_name
        tail_meta = {
        ...
        #     shuffle=False,
        # )
        FORGET_SET_KEY = head_split_name
        RETAIN_SET_KEY = tail_split_name
        return loaders
...
def run_weight_based_masking(c, hessian_coefficient = 0.5, retain_dataloader = False, retain_coefficient = 1, 
                            ...
                            normalize_by_layer = False, signs = "ABS", first_n = 600, classes = None):
    ...
    cripple_dataloader = bird_subset_eval_loaders.get(FORGET_SET_KEY)
    ...
    forget_dataloader_64_bs = bird_subset_eval_loaders.get(FORGET_SET_KEY)
...
def run_adapter_tuning(c):
    ...
    sys.stdout.flush()
    # fine_tune_vit(opt, train_loader_pers, val_loader_sketch, epochs = 100, lr=1e-4)
    # return 
    adapters = inject_adaptformer(opt)
    save_path = "best_adapter_16-of-100-epochs_0.0001-lr_74.62-valacc.pt"
    print(f"Loading adapter weights from {save_path}...")
    sys.stdout.flush()
    state_dict = torch.load(save_path, map_location=opt.device)

    # Load into the module list
...
def prune_adapter(c, path, print_personalization_scores = False, fisher_block_size = 64,
                     damp = 1e-3, eps = 1e-9, num_grads = 256, print_pruned_only_scores = True, switch_m = False, use_basis = False,
                     just_personalize = False, compensation_lr = 1, fisher_m_multiplier = 1, personalize_model = False):
