...
def custom_collate_exclude_skips(batch):
    # Filter out samples marked for skipping
    filtered = [sample for sample in batch if not sample.get("skip", False)]
    return default_collate(filtered)

# more efficient batched method
class LazyTransformDataset(Dataset):
    def __init__(self, hf_dataset, transform=None, indices=None, eval_config=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            if item[self.eval_config.dataset_image_key].mode == "L":
                return {"skip": True}
                return None

            item["image"] = self.transform(item[self.eval_config.dataset_image_key]) #TODO: probably need to be replaced with  eval_config.dataset_image_key rather than hardcoded.
            item["image"]["pixel_values"] = item["image"]["pixel_values"][0]
        else:
            print("NOT TRANSFORM")
...
class ImageGenerators(Generators):
    ...
    def get_image_classification_generator(
            ...
            for data in dataset:
                ...
                with torch.no_grad():
                    try:
                        ...
                        # print("Skipping image due to error.")  # TODO: uncomment this
                        continue
        ...
        lazy_dataset = LazyTransformDataset(dataset, transform, eval_config=eval_config)
        dataloader = DataLoader(lazy_dataset, batch_size=32, num_workers=8, collate_fn=custom_collate_exclude_skips, shuffle=True)

        # run batched data
        print(f"in evaluation len dataloader is {len(lazy_dataset)}")
        print(f"in evaluation dataloader batch size is {dataloader.batch_size}")
        count = 0
        for data in dataloader:
            count +=1
            print("in batched data run for evaluation")
            # predict next token from text
            img   = data[eval_config.dataset_image_key]
            inputs=img
            label = data[eval_config.dataset_image_label_key]
...
class Evaluator:
    ...
    def evaluate_dataset( self,
        ...
        print("in evaluate dataset")

        # Loop over the dataset
        pbar = tqdm(total=c.sample_size, miniters=1000, leave=False)
        for _item  in generator:
            ...
            if total_acc_data.num_skip_predictions > c.sample_size:
                print("In break of the num skip predicitons")
                break
...
def choose_functions(eval_config):
    print("in choose functions")
    print(f"eval_config datasettype is {eval_config.dataset_type}")
    if eval_config.dataset_name == "toxicity":
        print(f"it is toxicity")
    ...
    if eval_config.dataset_type == "image-membership-inference-attack":
        print(f"it is image-membership-inference-attack")
